...
export const useSocketStore = defineStore('socket', {
  ...
  actions: {
    ...
    async syncWalletState() {
      ...
      if (this.syncingWallet) {
        this.pendingWalletSync = true
        return
      }
      ...
      try {
        ...
        console.warn('[socket] wallet sync failed', error)
      } finally {
        this.syncingWallet = false
        if (this.pendingWalletSync) {
          this.pendingWalletSync = false
          queueMicrotask(() => {
            void this.syncWalletState()
          })
        }
      }
    },
    ...
    connect() {
      ...
      this.socket.on('wallet_update', (payload) => {
        const userStoreRef = useUserStore()
        if (typeof payload?.balance !== 'undefined') {
          userStoreRef.setBalance(payload.balance)
        }
        void this.syncWalletState()
      })
