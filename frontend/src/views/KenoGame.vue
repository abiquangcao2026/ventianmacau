<template>
  <section class="keno-page">
    ...
    <div v-if="maintenanceMode" class="keno-maintenance">
      <strong>KENO đang bảo trì</strong>
      <span>{{ maintenanceMessage }}</span>
    </div>

    <div v-else class="keno-bet">
      ...
      <div class="keno-bet__gates">
        <button class="keno-gate" type="button" :disabled="!canBet" @click="placeBet('tai')">
          Lớn x{{ formatOdds(gateOdds.tai) }}
        </button>
        <button class="keno-gate" type="button" :disabled="!canBet" @click="placeBet('xiu')">
          Nhỏ x{{ formatOdds(gateOdds.xiu) }}
        </button>
        <button class="keno-gate" type="button" :disabled="!canBet" @click="placeBet('even')">
          Chẵn x{{ formatOdds(gateOdds.even) }}
        </button>
        <button class="keno-gate" type="button" :disabled="!canBet" @click="placeBet('odd')">
          Lẻ x{{ formatOdds(gateOdds.odd) }}
        </button>
      </div>
...
const maintenanceMessage = ref('Game KENO đang bảo trì. Vui lòng quay lại sau.')
...
function onRoundState(payload) {
  ...
  gateOdds.tai = Number(payload?.config?.odds?.tai || gateOdds.tai || 1.98)
  gateOdds.xiu = Number(payload?.config?.odds?.xiu || gateOdds.xiu || 1.98)
  gateOdds.even = Number(payload?.config?.odds?.even || gateOdds.even || 1.98)
  gateOdds.odd = Number(payload?.config?.odds?.odd || gateOdds.odd || 1.98)
}
...
function onBettingSnapshot(payload) {
  if (!payload || payload.roomId !== roomId.value) return
  state.roundId = payload.roundId || state.roundId
}
...
function joinRoom(targetRoomId) {
  ...
  if (joinedRoomId.value && joinedRoomId.value !== targetRoomId) {
    socket.emit('leave_keno_room', { roomId: joinedRoomId.value })
  }

  socket.emit('join_keno_room', { roomId: targetRoomId }, (res) => {
    if (!res?.ok) {
      message.value = res?.message || 'Không thể vào phòng Keno'
      isError.value = true
      maintenanceMode.value = Boolean(res?.maintenance)
      if (res?.maintenance && res?.message) {
...
onMounted(() => {
  ...
  socket.on('keno_betting_snapshot', onBettingSnapshot)
})

watch(roomId, (nextRoomId) => {
  joinRoom(String(nextRoomId || 'keno-1p'))
})

onBeforeUnmount(() => {
  const socket = socketStore.socket
  if (!socket) return
  socket.emit('leave_keno_room', { roomId: joinedRoomId.value || roomId.value })
  ...
  socket.off('keno_betting_snapshot', onBettingSnapshot)
})
</script>
...
.keno-maintenance {
  ...
  padding: 22px 18px;
  border-radius: 16px;
  background: linear-gradient(135deg, rgba(104, 14, 14, 0.9), rgba(146, 34, 34, 0.86));
  border: 1px solid rgba(255, 215, 215, 0.18);
  display: flex;
...
.keno-maintenance strong {
  font-size: 18px;
}

.keno-maintenance span {
